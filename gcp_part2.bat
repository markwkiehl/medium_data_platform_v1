...
echo.
echo This batch file will setup a new GCP Project with the above settings (check them carefully).  
echo The Google Cloud Billing Account No. %GCP_BILLING_ACCOUNT% will be linked to the project %GCP_PROJ_ID%.
echo The user-manged service account %GCP_SVC_ACT% 
echo will be created for the project %GCP_PROJ_ID%.
...
IF %ERRORLEVEL% NEQ 0 (
	echo ERROR %ERRORLEVEL%: gcloud iam service-accounts create %GCP_SVC_ACT_PREFIX% --description="user-managed service account" --display-name="%GCP_SVC_ACT_PREFIX%"
	EXIT /B
)
...
echo You may continue with gcp_part3.bat